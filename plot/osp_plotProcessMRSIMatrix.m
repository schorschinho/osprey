function out = osp_plotProcessMRSIMatrix(MRSCont, kk, which_spec,coord, ppmmin, ppmmax,lb,add_No_MoCo,which_slice,yLim,mask,amplitude)
...
if nargin <12
    amplitude = 0;
    if nargin <11
        mask = ones(MRSCont.processed.(which_spec{1}){kk}.nXvoxels,MRSCont.processed.(which_spec{1}){kk}.nYvoxels);
        if nargin <10
            yLim = [];
            if nargin < 9
                which_slice =1;
...
% mask = fliplr(rot90(mask(:,:,which_slice),2));
...
if MRSCont.flags.hasRef || MRSCont.flags.hasWater
    ...
    if isempty(yLim)
        yLim = [-max(MRSCont.plot.processed.(which_spec{1}).max)/8 max(MRSCont.plot.processed.(which_spec{1}).max)/8]/Norm;
    ...
    if strcmp(which_spec{1},'w')
         Norm = MRSCont.plot.processed.w.ContMax;
    end
    if isempty(yLim)
        yLim = [min(MRSCont.plot.processed.A.min) max(MRSCont.plot.processed.A.max)*2]/Norm;
    end
end
...
%         out = subaxis(coord(2,2)-coord(2,1)+1,coord(1,2)-coord(1,1)+1,plot_index, 'Spacing', 0.01, 'Padding', 0, 'Margin', 0, 'MarginLeft', 0.075, 'MarginBottom', 0.1);
        ...
        for ss = 1 : length(which_spec)
            if add_No_MoCo
                ...
                if ~amplitude
                    ...
                    plot(procNoMoCoData.ppm,real(procNoMoCoData.specs)/Norm,'Color',colormap.LightAccent{1});
                end
            end
            ...
            if ~amplitude
                ...
                if ss == 1
                    plot(procData.ppm,real(procData.specs)/Norm,'Color',colormap.Accent);    
                else
                    plot(procData.ppm,real(procData.specs)/Norm,'Color',colormap.Accent);   
                end
            end
        ...
        if amplitude
            scales = max(abs(real(procData.specs))/Norm);
            set(gca,'Color',[scales, scales, scales]);
        end
%         text(ppmmax,yLim(2),[num2str(y) ' ' num2str(x)]);
