    ...
    if ~(MRSCont.flags.didSeg == 1 && MRSCont.flags.speedUp && isfield(MRSCont, 'seg') && (kk > length(MRSCont.seg.tissue.fGM))) || ~strcmp(MRSCont.ver.Osp,MRSCont.ver.CheckOsp)
        ...
        if ~exist(segFile,'file')
            ...
            if exist(fullfile(T1dir, ['c1' T1name '.nii.gz']),'file')
                ...
                if MRSCont.flags.isMRSI
                    gunzip(fullfile(T1dir, ['c5' T1name T1ext '.gz'])); 
                end
            end
        ...
        for rr = 1 : Voxels % This also loops over the different slice for MRSI
            ...
            if MRSCont.flags.isMRSI
                ...
                for y = 1 : MRSCont.raw{kk}.nYvoxels
                    for x = 1 : MRSCont.raw{kk}.nXvoxels
                        ...
                        if MRSCont.opts.MRSI.outerMask.mask(x,y,rr)
                            ...
                            GM_voxmask_vol = GMvol .* index_mask_temp;
                            WM_voxmask_vol = WMvol .* index_mask_temp;
                            CSF_voxmask_vol = CSFvol .* index_mask_temp;
                            LIP_voxmask_vol = LIPvol .* index_mask_temp;
                            
                            GMsum  = sum(GM_voxmask_vol(GM_voxmask_vol>0.9),'all');
                            WMsum  = sum(WM_voxmask_vol(WM_voxmask_vol>0.9),'all');
                            CSFsum = sum(CSF_voxmask_vol(CSF_voxmask_vol>0.9),'all');
                            LIPsum = sum(LIP_voxmask_vol(LIP_voxmask_vol>0.9),'all');
                ...
                temp = MRSCont.seg.tissue.fGM(kk,:,:,:) + MRSCont.seg.tissue.fWM(kk,:,:,:);
                ...
                spm_imcalc({GMvol.fname; WMvol.fname; CSFvol.fname}, fullfile(T1dir, [T1name '_brain' T1ext]), '(i1+i2+i3)>0.1');
