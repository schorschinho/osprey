classdef parfor_wait < handle
   ...
   methods
       function Obj = parfor_wait(TotalMessage, varargin)
           ...
           addParameter(InParser,'CurrentStep','Modeling',@ischar);
           ...
           Obj.CurrentStep = InParser.Results.CurrentStep;
           if Obj.Waitbar
   ...
   methods (Access = private)
       ...
       function WaitbarUpdate(Obj)
           ...
           waitbar(Obj.NumMessage/Obj.TotalMessage, Obj.WaitbarHandle, [Obj.CurrentStep ' ' num2str(Obj.NumMessage/Obj.TotalMessage*100, '%.2f'), '%; ', num2str(UsedTime_now, '%.2f'), 's used and ', num2str(EstimatedTimeNeeded, '%.2f'), 's needed.']);
       ...
       function FileUpdate(Obj)
           ...
           switch Obj.FileName
               case 'screen'
                   fprintf('%s %.2f%%; %.2fs used and %.2fs needed...\n',Obj.CurrentStep, Obj.NumMessage/Obj.TotalMessage*100, UsedTime_now, EstimatedTimeNeeded);
               otherwise
                   fprintf(Obj.FileID, '%s %.2f%%; %.2fs used and %.2fs needed...\n', Obj.CurrentStep, Obj.NumMessage/Obj.TotalMessage*100, UsedTime_now, EstimatedTimeNeeded);
           end
           Obj.UsedTime_1 = UsedTime_now;
       end
       function AddOne(Obj)
