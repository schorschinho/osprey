...
function [out,lor]=op_filter_Gauss(in,lb);
...
if lb==0
    ...
    % GaussianLWfactor = (1/2*(pi*sqrt(2*log(2)))^2);  
    GaussianLWfactor = 1/2; 

    t2=GaussianLWfactor*lb^2;
    
    %Create an gaussian decay (gaussian filter):
    lor=exp(-in.t.*in.t*t2);
    %plot(in.t,lor);
