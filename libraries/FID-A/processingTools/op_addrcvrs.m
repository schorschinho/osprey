...
if in.nXvoxels*in.nYvoxels*in.nZvoxels == 1                 %            SVS
    ...
    phs=squeeze(coilcombos.ph);
    sigs=coilcombos.sig;

    if in.nZvoxels == 1
        ph = repmat(phs, [1 1 1 in.sz(in.dims.t)]);
        ph = permute(ph,[4 1 2 3]);
        sig = repmat(sigs, [1 1 1 in.sz(in.dims.t)]);
        sig = permute(sig,[4 1 2 3]);
    else
        ph = repmat(phs, [1 1 1 1 in.sz(in.dims.t)]);
        ph = permute(ph,[5 1 2 3 4]);
        sig = repmat(sigs, [1 1 1 1 in.sz(in.dims.t)]);
        sig = permute(sig,[5 1 2 3 4]);
    end
...
if in.dims.Xvoxels>in.dims.coils
    dims.Xvoxels=in.dims.Xvoxels-1;
else
    dims.Xvoxels=in.dims.Xvoxels;
end
if in.dims.Yvoxels>in.dims.coils
    dims.Yvoxels=in.dims.Yvoxels-1;
else
    dims.Yvoxels=in.dims.Yvoxels;
end
if in.dims.Zvoxels>in.dims.coils
    dims.Zvoxels=in.dims.Zvoxels-1;
else
    dims.Zvoxels=in.dims.Zvoxels;
end
