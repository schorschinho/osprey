...
% 
% USAGE:
% parsFit=op_naaFit(in,ph0,ph1);
% 
% DESCRIPTION:
% Perform a Lorentzian lineshape fit to the NAA resonance in a brain 
% proton MRS dataset.
% 
% INPUTS:
...
%
% OUTPUTS:
% parsFit  = Fit parameters for the NAA peak fit.  
...
function parsFit=op_naaFit(in,ph0,ph1);
...
parsGuess=zeros(1,4);
[y_extr, idx_extr] = max(abs(real(spec)));
parsGuess(1)=real(spec(idx_extr)); %Amplitude
parsGuess(2)=3/(42.577*in.Bo); %Linewidth [Hz]
parsGuess(3)= ppm(idx_extr); %Frequency [ppm];
