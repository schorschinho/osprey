...
%Heiner N. Raum, UKM Muenster 2024 (adaption of geometry header fields from XA30 to VE).
%
% USAGE:
...
function [out] = io_loadspec_rda(pathname)
    ...
    for pp = 1:length(VoI_Params)
        if ~isfield(rda, VoI_Params{pp})
            ...
            %Modified HR%
            flag.(VoI_Params{pp}) = false;
            %
        else 
            flag.(VoI_Params{pp}) = true;
        end
    end
    
    % Write them
    %Modified HR%
    if contains(rda.software_version,'XA30','IgnoreCase',true) && ~any(structfun(@(x) x == true,flag))
        ...
        PhaseRot = rda.SlabOrientation(2,:)';          
        % swapped signs because VoI_InPlaneRot rotates counterclockwise
        % around Norm
        if dot(cross(Phase,PhaseRot),Norm) <= 0
            geometry.rot.VoI_InPlaneRot = acos(dot(Phase,PhaseRot));
        else
            geometry.rot.VoI_InPlaneRot = -acos(dot(Phase,PhaseRot));
        end
        %End of calculating InPlaneRot
    else
