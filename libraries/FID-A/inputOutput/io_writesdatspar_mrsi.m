...
function RF=io_writesdatspar_mrsi(in,infile,outfile);
%function RF=io_writesdatspar(in,outfile);
[~, header] = philipsLoad(infile);
...
elseif length(in.sz) == 4
    reshape_data = reshape(fids, [1 in.sz(1) in.sz(2) in.sz(3) in.sz(4)]);
end
...
fprintf(fid,'\n');
fprintf(fid,'phase_encoding_enable : "yes"\n');
fprintf(fid,'\n');
fprintf(fid,'nr_phase_encoding_profiles :  %i\n', in.sz(3));
fprintf(fid,'\n');
fprintf(fid,'si_ap_off_center : %4.8f\n', header.si_ap_off_center);
fprintf(fid,'\n');
fprintf(fid,'si_lr_off_center : %4.8f\n', header.si_lr_off_center);
fprintf(fid,'\n');
fprintf(fid,'si_cc_off_center : %4.8f\n', header.si_cc_off_center);
fprintf(fid,'\n');
fprintf(fid,'si_ap_off_angulation : %4.8f\n', header.si_ap_off_angulation);
fprintf(fid,'\n');
fprintf(fid,'si_lr_off_angulation : %4.8f\n', header.si_lr_off_angulation);
fprintf(fid,'\n');
fprintf(fid,'si_cc_off_angulation : %4.8f\n', header.si_cc_off_angulation);
fprintf(fid,'\n');
fprintf(fid,'t0_kx_direction : %i\n', header.t0_kx_direction);
fprintf(fid,'\n');
fprintf(fid,'t0_ky_direction : %i\n', header.t0_ky_direction);
fprintf(fid,'\n');
fprintf(fid,'nr_of_phase_encoding_profiles_ky : %i\n', in.sz(3));
fprintf(fid,'\n');
fprintf(fid,'phase_encoding_direction : "trans"\n');
fprintf(fid,'\n');
fprintf(fid,'phase_encoding_fov : %i\n', header.phase_encoding_fov);
fprintf(fid,'\n');
fprintf(fid,'slice_thickness : %i\n', header.slice_thickness);
fprintf(fid,'\n');
fprintf(fid,'image_plane_slice_thickness : 0\n');
fprintf(fid,'\n');
fprintf(fid,'slice_distance : %i\n',header.slice_distance);
fprintf(fid,'\n');
fprintf(fid,'nr_of_slices_for_multislice : %i\n',header.nr_of_slices_for_multislice);
fprintf(fid,'\n');
...
fprintf(fid,'!-------------------------------------------------\n');
fprintf(fid,'\n');
fprintf(fid,'dim4_ext :[num]\n');
fprintf(fid,'\n');
fprintf(fid,'dim4_pnts : %i\n',in.sz(4));
fprintf(fid,'\n');
fprintf(fid,'dim4_low_val : 1.000000\n');
fprintf(fid,'\n');
fprintf(fid,'dim4_step : 1.000000\n');
fprintf(fid,'\n');
fprintf(fid,'dim4_direction : slice\n');
fprintf(fid,'\n');
fprintf(fid,'dim4_t0_point : -\n');
fprintf(fid,'\n');
fprintf(fid,'!-------------------------------------------------\n');
fprintf(fid,'\n');
