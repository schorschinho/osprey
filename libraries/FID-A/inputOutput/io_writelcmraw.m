...
idx  = find(strcmp(data_struct.name,metab));

expname=[data_struct.seq{1} num2str(data_struct.te) '_' data_struct.name{idx} 'Sim'];
comment=['Simulated LCModel basis set for ' data_struct.name{idx} ' ' data_struct.seq{1}...
...
f = (4.65-data_struct.centerFreq)*hzpppm;
...
fprintf(fid,'\n\n $NMID ID=%s',data_struct.name{idx});
fprintf(fid,'\n FMTDAT=(2E16.6)');
fprintf(fid,'\n VOLUME=1.00000');
fprintf(fid,'\n TRAMP=1.00000');
fprintf(fid,'\n $END');
