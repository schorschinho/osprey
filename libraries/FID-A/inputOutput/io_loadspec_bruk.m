...
% verbose = Flag whether command line outputs are generated. Default = 0
% 
% OUTPUTS:
...
function [out,ref]=io_loadspec_bruk(inDir, verbose)
...
try
    ...
    if verbose
        fprintf('WARNING: /fid.raw not found. Using /fid ....')
    end
    ...
    if mod(size(real_fid,1),rawDataPoints) ~=0
        if verbose
            fprintf('number of repetitions cannot be accurately found');
        end
    end
...
if exist([inDir '/fid.ref'])
    ...
    refspecs=fftshift(fft(reffids,[],1),1);
    sz=size(refspecs);
    out.flags.averaged=0;
    %specify the dims
    refdims.t=1;
...
method_fid=fopen([inDir '/method']);
line=fgets(method_fid);
index=findstr(line,'$Bvalue=');
while isempty(index)
    line=fgets(method_fid);
    index=findstr(line,'$Bvalue=');
end
equals_index=findstr(line,'=');
bValue=line(equals_index+1:end);
bValue=str2double(bValue);
fclose(method_fid);
...
method_fid=fopen([inDir '/method']);
line=fgets(method_fid);
index=findstr(line,'$PVM_VoxArrSize=');
while isempty(index)
    line=fgets(method_fid);
    index=findstr(line,'$PVM_VoxArrSize=');
end
...
method_fid=fopen([inDir '/method']);
line=fgets(method_fid);
index=findstr(line,'$PVM_VoxArrPosition=');
while isempty(index)
    line=fgets(method_fid);
    index=findstr(line,'$PVM_VoxArrPosition=');
end
line=fgets(method_fid);
voxelSize = sscanf(line, '%f');
fclose(method_fid);

% Geometry
% Put rotation on hold for now...
geometry.size.ap = voxelDims(2); % voxel size in AP direction [mm]
geometry.size.lr = voxelDims(1); % voxel size in LR direction [mm]
geometry.size.cc = voxelDims(3); % voxel size in CC direction [mm]
geometry.pos.ap = voxelSize(2); % voxel center offset in AP direction [mm]
geometry.pos.lr = voxelSize(1); % voxel center offset in LR direction [mm]
geometry.pos.cc = voxelSize(3); % voxel center offset in CC direction [mm]
...
% Bruker data assumes the center frequency to be 4.7 ppm:
centerFreq = 4.7;
ppm=ppm + centerFreq;
