...
function [splineArray] = osp_gLCM_makeSplineBasis(dataToFit, fitRangePPM, dkntmn, plotFlag)
...
% enter the LCModel.
nKnotsNominal = floor((fitRangePPM(end) - fitRangePPM(1))/dkntmn) + 1 + 2;
...
baselineRangeExtended = linspace(1, floor(nPtsBaseLineExtended), floor(nPtsBaseLineExtended));
...
splineBasisFunctionMatrix = splineBasisFunctionMatrix(:,:,1) + 1j.*splineBasisFunctionMatrix(:,:,2);

% Now cut the functions out only over the length of the frequency axis
eitherSide = round((length(baselineRangeExtended) - length(dataToFit.ppm))/2);

% We append with zeros and do a circshift so we can comfortably move the
% spline array along
appendWith = zeros(5000, size(splineBasisFunctionMatrix,2));
...
% crop
splineInnerBasisFunctionMatrix = splineBasisFunctionMatrix(5000+eitherSide+1:5000+eitherSide+length(dataToFit.ppm),:);
