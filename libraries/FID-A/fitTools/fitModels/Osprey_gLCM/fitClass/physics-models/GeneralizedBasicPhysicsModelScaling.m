...
%% Calculate lossfunction
    ...
    [~,PenaltyTermScaling]     = forwardJacobian(x,...
                          data,...
                          NoiseSD,...
                          basisSet,...
                          baselineBasis,...
    ...
    residual  = cat(1,residual,penaltyTermSoftConstraint*PenaltyTermScaling); % Add soft constraint penalty term
...
function [jac, PenaltyTermScaling] = forwardJacobian(x, data, NoiseSD, basisSet, baselineBasis, ppm, t, fitRange,SignalPart,Reg, parametrizations)
...
%% Calculate forward jacobian matrix
    ...
    residual = reshape(residual(indMin:indMax,:),[],1);
    % PenaltyTermScaling = norm(jac' * residual);
    PenaltyTermScaling = norm(residual);
    ...
    if secDim == 1                                                          % Add penalty term for soft constraints

        nParams = length(x);
        tempTerm = [];
        [~,ph0PenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'ph0', sD, nParams, PenaltyTermScaling);
        [~,ph1PenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'ph1', sD, nParams, PenaltyTermScaling);
        [~,gaussLBPenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'gaussLB', sD, nParams, PenaltyTermScaling);
        [~,lorentzLBPenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'lorentzLB', sD, nParams, PenaltyTermScaling);
        [~,freqShiftPenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'freqShift', sD, nParams, PenaltyTermScaling);
        [~,metAmplPenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'metAmpl', sD, nParams, PenaltyTermScaling);
        if nBaselineComps ~= 0 
            [~,baseAmplPenaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, 'baseAmpl', sD, nParams, PenaltyTermScaling);
        else
            baseAmplPenaltyJac = [];
        end
...
%% Calculate forward model and regularizer
    ...
    if secDim == 1
        nParams = length(x);
        ph0SoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'ph0', sD, nParams, 1);
        ph1SoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'ph1', sD, nParams, 1);
        gaussLBSoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'gaussLB', sD, nParams, 1);
        lorentzLBSoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'lorentzLB', sD, nParams, 1);
        freqShiftSoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'freqShift', sD, nParams, 1);
        metAmplSoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'metAmpl', sD, nParams, 1);
        if ~isempty(bl) 
            baseAmplSoftContstraint = calcSoftConstraintPenalty(inputParams, parametrizations, 'baseAmpl', sD, nParams, 1);
        else
...
function [penalty, penaltyJac] = calcSoftConstraintPenalty(inputParams, parametrizations, param, sD, nParams, PenaltyTermScaling)
penalty = [];
penaltyJac = [];
if ~isempty(parametrizations.(param).sc)
    ...
    penalty(parametrizations.(param).sc.adj_idx) = PenaltyTermScaling*diffVec./sdVec;
        ...
        for m_adj = 1 : size(parametrizations.(param).sc.adj_idx,1)
            if fixValue(m_adj) ~= 0
                ...
                for m_fix = 1 : length(fix_idx)
                    penaltyJac(parametrizations.(param).start+adj_idx-1,parametrizations.(param).start+fix_idx(m_fix)-1) = -PenaltyTermScaling*parametrizations.(param).sc.fix_factor{m_adj}(m_fix)*adjValue(m_adj)./(sdVec(m_adj).*fixValue(m_adj).*fixValue(m_adj));
                end
                penaltyJac(parametrizations.(param).start+adj_idx-1,parametrizations.(param).start+adj_idx-1) = PenaltyTermScaling*1./(sdVec(m_adj).*fixValue(m_adj)); 
            end
        end
    end
