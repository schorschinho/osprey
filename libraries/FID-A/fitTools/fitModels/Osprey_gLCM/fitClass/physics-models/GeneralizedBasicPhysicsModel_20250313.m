...
%% Calculate forward jacobian matrix
    ...
    dYdpen  = updateAccordingToGrouping(1,dYdpen,[], parametrizations,x);
...
if jacobian
    ...
    % end 
    % dYdX = dYdXtemp;
    dYdXtemp = zeros(size(dYdX,1),size(dYdX,2),size(dYdX,3));
    ...
    run_idx = 1;
     for ff = 1 : length(pars)                                               % Loop over parameters
        ...
        %             run_idx = run_idx + 1;
        %          end                
        %          if length(par_idx)/size(dYdX,3) == par_run_idx
        %             run_idx = run_idx_cur;
        %             par_run_idx = 1;
        %          end
        %         par_run_idx = par_run_idx +1;
        %     end
        % else   
            for ll = 1 : length(idx)
                if par_idx(1) ~= 0         
    ...
    if ndims(dYdX)==2
        dYdXtemp = zeros(size(dYdX,1),size(dYdX,2));
        pars = fields(parametrizations);
        run_idx = 1;
         for ff = 1 : length(pars)                                               % Loop over parameters
            par_idx = [parametrizations.(pars{ff}).start : parametrizations.(pars{ff}).end];
...
%% Update jacobian blocks
    ...
    if strcmp(parameterName,'ph0') ||  strcmp(parameterName,'ph1') || strcmp(parameterName,'gaussLB')
