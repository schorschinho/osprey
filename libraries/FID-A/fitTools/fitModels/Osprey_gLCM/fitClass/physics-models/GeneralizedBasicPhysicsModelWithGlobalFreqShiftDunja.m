...
function fh = GeneralizedBasicPhysicsModelWithGlobalFreqShift
...
%% Calculate forward jacobian matrix
    ...
    if ~isempty(fitGap)
        ...
        dYdGlobFreqShift((GapindMin-indMin):(GapindMax-indMin),:,:)   = []; % Cut out fit gap
    ...
    if ~isempty(NoiseSD)                                     % 1D jacobians have not been normalized yet
        Sigma = NoiseSD.FD;                                     % Set sigma
        Sigma = repmat(Sigma', [1 size(dYdpen,1) size(dYdpen,2)]);                         % Repeat according to dimensions
        Sigma = permute(Sigma,[2 3 1]);                                     % Dims have to be nPoints nLines secDim
        dYdpen = dYdpen .* Sigma;                            % Normalize jacobian
    end
    
    [dYdph0]        = cat(1,dYdph0,-dYdpen(:,1,:)) ;                                % Add regularizer to ph0 parameter
    [dYdph1]        = cat(1,dYdph1,-dYdpen(:,2,:));                                 % Add regularizer to ph1 parameter
    [dYdGlobFreqShift] = cat(1,dYdGlobFreqShift,-dYdpen(:,3,:));                    % Add regularizer to GlobFreqShift parameter
    [dYdgaussLB]    = cat(1,dYdgaussLB,-dYdpen(:,4,:));                             % Add regularizer to gaussLB parameter
    [dYdlorentzLB]  = cat(1,dYdlorentzLB,-dYdpen(:,5:5+nBasisFcts-1,:));                % Add regularizer to lorentzLB parameter
    [dYdfreqShift]  = cat(1,dYdfreqShift,-dYdpen(:,5+nBasisFcts:5+2*nBasisFcts-1,:));               % Add regularizer to freqShift parameter
    [dYdmetAmpl]    = cat(1,dYdmetAmpl,-dYdpen(:,5+nBasisFcts*2:5+3*nBasisFcts-1,:));                 % Add regularizer to metAmpl parameter
    if nBaselineComps ~= 0
        [dYdbaseAmpl]   = cat(1,dYdbaseAmpl,-dYdpen(:,5+3*nBasisFcts:end,:));           % Add regularizer to baseAmpl parameter
    end
...
%% Calculate forward model and regularizer
    ...
    penaltyTerm = calcPenalty([],inputParams, parametrizations, secDim);
...
%% Add regularizer to parameter matrix
    if ~strcmp(parametrizations.(parameterName).RegFun,'')                  % Don't apply regularization to the parameter
        if jacobian
           ...
           RegMatrix = repmat(sqrt(parametrizations.(parameterName).RegPar)*RegMatrix,[1 1 secDim]);
...
for sD = 1 : secDim                                                 % Loop over indirect dimension
    ...
    for pp = 1:length(params)
        ...
        actualValue         = squeeze(inputParams.(params{pp})(sD,:));
        if strcmp(parametrizations.(params{pp}).type,'free')
            expectationValue    = parametrizations.(params{pp}).ex(sD,:);
            standardDeviation   = parametrizations.(params{pp}).sd(sD,:);
        end
        if strcmp(parametrizations.(params{pp}).type,'fixed')
            expectationValue    = parametrizations.(params{pp}).ex(1,:);
            standardDeviation   = parametrizations.(params{pp}).sd(1,:);
        end
        ...
        if isempty(jacobian) 
            if ~strcmp(params{pp},'baseAmpl')                               % The baseline parameter might be empty
                ...
                if ~strcmp(parametrizations.(params{pp}).type,'none')       % Add penalty term if baseline is defined
