%%  GeneralizedBasicPhysicsModelMaxEchoSym.m
...
function fh = GeneralizedBasicPhysicsModelMaxEchoSym
...
%% Calculate forward jacobian matrix
    ...
    for sD = 1 : secDim                                                     % Loop over indirect dimension
        ...
        for ll = 1:nBasisFcts                                               % Loop over basis functions
            % timeDomainMultiplierRight(:,ll) = exp(-(1i*2*pi*freqShift(ll) + (1/pi)*lorentzLB(ll) + GaussianLWfactor * gaussLB.^2.*t(1:end-basisSet.pointsToPick+1)).*t(1:end-basisSet.pointsToPick+1))';  % Apply freqshifts, lorentzianLBright, and gaussLB
            % timeDomainMultiplierLeft(:,ll) = exp(-(1i*2*pi*freqShift(ll) + (1/pi)*lorentzLB(ll) + GaussianLWfactor * gaussLB.^2.*t(1:basisSet.pointsToPick-1)).*t(1:basisSet.pointsToPick-1))';  % Apply freqshifts, lorentzianLBleft, and gaussLB
            % timeDomainMultiplier(:,ll) = cat(1,flip(timeDomainMultiplierLeft(:,ll)),timeDomainMultiplierRight(:,ll));

            timeDomainMultiplier(:,ll) = exp(-(1i*2*pi*freqShift(ll) + (1/pi)*lorentzLB(ll) + GaussianLWfactor * gaussLB.^2.*t).*t)';  % Apply freqshifts, lorentzianLBright, and gaussLB
