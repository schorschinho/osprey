...
function [fitParams, resBasisSet] = fit_OspreyNoLS(dataToFit, basisSet, fitOpts)
...
dataToFitRef.valued = fitOpts.valued;
...
if ~isfield(fitOpts, 'MRSIpriors')
    ...
    [fitParamsStep1] = fit_OspreyNoLS_PrelimReduced(dataToFitRef, resBasisSet, [1.8 4],refShift,refFWHM);
else
...
[fitParamsStep2] = fit_OspreyNoLS_PrelimKnotSpacing(dataToFitRef, resBasisSet, fitRangePPM, fitParamsStep1);
delete('ampl.mat');
%%% 5. FINAL PRELIMINARY ANALYSIS STEP 2 %%%
...
if isfield(fitOpts,'coMM3') && isfield(dataToFit,'refShift') && ~isfield(fitOpts,'Diff2')
    switch fitOpts.coMM3        
        case '3to2MMsoft'
        [fitParamsStep3] = fit_OspreyPrelimStep2coMM3(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM,0.66,'MM');
        case '1to1GABAsoft'
        [fitParamsStep3] = fit_OspreyPrelimStep2coMM3(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM,1,'GABA');
        case '3to2GABAsoft'  % 3:2 GABA and co-edited MM3 model
        [fitParamsStep3] = fit_OspreyPrelimStep2coMM3(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM,0.66,'GABA');
        case '2to3GABAsoft' % 2:3 GABA and co-edited MM3 model
        [fitParamsStep3] = fit_OspreyPrelimStep2coMM3(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM,1.5,'GABA');
        case 'freeGauss' % Gauss with free frequency and linewidth
        [fitParamsStep3] = fit_OspreyPrelimStep2freeGauss(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, fitParamsStep2);
        resBasisSet = fitParamsStep3.resBasisSetUpdated;
        otherwise
            [fitParamsStep3] = fit_OspreyNoLSPrelimStep2(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, fitParamsStep2);
    end             
else
    [fitParamsStep3] = fit_OspreyNoLSPrelimStep2(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, fitParamsStep2);    
end

% [CRLB] = fit_OspreyNoLS_CRLB(resBasisSet,fitParamsStep3);



%%% 5. CREATE OUTPUT %%%
% Return fit parameters
fitParams = fitParamsStep3;
fitParams.refShift = refShift;
fitParams.refFWHM = refFWHM;
fitParams.prelimParams = fitParamsStep1;
fitParams.prelimParams2 = fitParamsStep2;
% fitParams.CRLB = CRLB;
end
