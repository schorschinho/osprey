function [fitParamsStep3] = fit_OspreyPrelimStep3(dataToFit, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep2,GAP)
%% [fitParamsStep2] = fit_OspreyPrelimStep3(dataToFit, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM)
...
inputSettings.ph0         = fitParamsStep2.ph0; % zero-order phase correction [deg]
inputSettings.ph1         = fitParamsStep2.ph1; % first-order phase correction [deg/ppm]
inputSettings.gaussLB     = fitParamsStep2.gaussLB; % Gaussian dampening [Hz]
inputSettings.freqShift   = fitParamsStep2.freqShift(1:end-1); % Lorentzian dampening [Hz] for each basis function
gaussLBMM     = fitParamsStep2.gaussLBMM; % Common Gaussian dampening [Hz]
inputSettings.lorentzLB   = fitParamsStep2.lorentzLB(1:end-1); % Lorentzian dampening [Hz] for each basis function
lorentzLB_MM   = fitParamsStep2.lorentzLB(end) * ones(resBasisSet.nMM,1);
freqShift_MM   = zeros(resBasisSet.nMM,1); % Frequency shift [Hz] for each basis function
inputSettings.ampl        = fitParamsStep2.ampl(1 : end-1); % Amplitude parameters for basis functions and baseline
ampl   = zeros(resBasisSet.nMM+nSplines,1); % Frequency shift [Hz] for each basis function
lineShape        = fitParamsStep2.lineShape; % Amplitude parameters for basis functions and baseline


% Normalize
inputSettings.lineShape = lineShape/sum(lineShape);

% Concatenate all initial guesses together into one large x0 vector.
x0          = [gaussLBMM; lorentzLB_MM; freqShift_MM; ampl];
...
nMM     = resBasisSet.nMM;
lb_gaussLBMM          = 0; 
ub_gaussLBMM          = sqrt(5000); % Gaussian dampening [Hz]
lb_lorentzLB_MM     = zeros(nMM, 1); 
ub_lorentzLB_MM     =  100 * ones(nMM, 1); % Lorentzian dampening [Hz] - MM/Lipids
lb_freqShift_MM     = -4 * ones(nMM,1); 
ub_freqShift_MM     = +4 * ones(nMM,1); % Frequency shift [Hz] - MM/Lipids
lb_ampl             = -Inf * ones(nMM+size(splineArray,2),1); 
ub_ampl             = +Inf * ones(nMM+size(splineArray,2),1); % Amplitude for metabolite and spline basis functions


% Concatenate together into LB/UB vectors
lb = [lb_gaussLBMM; lb_lorentzLB_MM; lb_freqShift_MM; lb_ampl];
ub = [ub_gaussLBMM; ub_lorentzLB_MM; ub_freqShift_MM; ub_ampl];
...
[x,Res,~,~,~,~,~,J] = LevenbergMarquardt(@(x) fit_Osprey_PrelimStep3_Model(x, inputData, inputSettings), x0, lb, ub, opts);
...
% baseline parameters).
[fitParamsFinal] = fit_Osprey_PrelimStep3_finalLinear(x, inputData, inputSettings);
...
function [F,J] = fit_Osprey_PrelimStep3_Model(x, inputData, inputSettings)
...
ph0         = inputSettings.ph0 * pi/180; % zero-order phase correction [convert from deg to rad]
ph1         = inputSettings.ph1 * pi/180; % first-order phase correction [convert from deg/ppm to rad/ppm]
gaussLB     = inputSettings.gaussLB; % Gaussian dampening [Hz^2]
lorentzLB   = inputSettings.lorentzLB;
freqShift   = inputSettings.freqShift;
gaussLBMM     = x(1); % Gaussian dampening [Hz^2]
lorentzLBMM   = x(2:nMM+1); % Lorentzian dampening [Hz] for each basis function
freqShiftMM   = x(nMM+2:2*nMM+1); % Frequency shift [Hz] for each basis function
ampl        = x(2*nMM+2:3*nMM+1+nSplines); % Amplitudes
% Normalize the lineshape
lineShape = inputSettings.lineShape;
...
for ii=1:resBasisSet.nMM
    resBasisSet.fids(:,resBasisSet.nMets+ii) = resBasisSet.fids(:,resBasisSet.nMets+ii) .* exp(-1i*freqShiftMM(ii).*t)' .* exp(-lorentzLBMM(ii).*t)' .* exp(-gaussLBMM.*t.*t)';    
    resBasisSet.fids(:,resBasisSet.nMets+ii) = resBasisSet.fids(:,resBasisSet.nMets+ii) * exp(1i*ph0);
end
...
A = real(resBasisSet.specs(:,1:resBasisSet.nMets));
for kk = 1:resBasisSet.nMets
    A(:,kk) = conv(A(:,kk), lineShape, 'same');
end
A_us = A;
A = A * inputSettings.ampl;

MM = real(resBasisSet.specs(:,resBasisSet.nMets+1:end));
...
l = [zeros(nMM,1);    -inf*ones(nSplines,1)];
u = [inf*ones(nMM,1);  inf*ones(nSplines,1)];
...
ampl = vertcat(inputSettings.ampl,ampl);
...
regBAmpl = sum((beta_j.^2))/sqrt(stdNoise);

F = (data - [A_us AB]*ampl) + regBAmpl;
...
function [fitParamsFinal] = fit_Osprey_PrelimStep3_finalLinear(x, inputData, inputSettings)
...
fitParamsFinal.ampl         = ampl(1:nBasisFcts);
fitParamsFinal.ph0          = inputSettings.ph0;
fitParamsFinal.ph1          = inputSettings.ph1;
fitParamsFinal.gaussLB      = inputSettings.gaussLB;
fitParamsFinal.gaussLBMM      = x(1);
fitParamsFinal.lorentzLB    = [inputSettings.lorentzLB; x(2:nMM+1)];
fitParamsFinal.freqShift    = [inputSettings.freqShift; x(nMM+1:2*nMM+1)];
fitParamsFinal.lineShape    = inputSettings.lineShape;
fitParamsFinal.beta_j       = ampl(nBasisFcts+1:end);
