...
opts.MaxIter    = 400;
opts.Jacobian   = 'on';
[x,Res,~,~,~,~,~,~] = LevenbergMarquardt(@(x) fit_Osprey_PrelimStep2_Model(x, inputData, inputSettings), x0, lb, ub, opts);
...
for ii=1:nLineShape % Loop over the lineshape derivatives
    ...
    for kk = 1 : nMets % Convolute all basis functions to create the full model function
        ...
        Jfd(:,col) = Jfd(:,col) + ampl(ii)*(ToepLineShape(:,:,ii) * fftshift(fft(resBasisSet.fids(:,kk),[],1),1));
    end
