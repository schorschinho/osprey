...
function [fitParams] = fit_Osprey(dataToFit, resBasisSet, fitOpts)
...
[x,~,~,~,~,~,~,~] = LevenbergMarquardt(@(x) fit_nlinOsprey(x, dataToFit, resBasisSet, spl_pos, fitRangePPM),x0,lb,ub,opts);
...
% the baseline.
[appliedBasisSet, B] = fit_applynlinOsprey(resBasisSet, x, spl_pos, fitRangePPM);
