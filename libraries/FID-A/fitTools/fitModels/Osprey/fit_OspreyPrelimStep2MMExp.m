function [fitParamsStep2] = fit_OspreyPrelimStep2MMExp(dataToFit, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1, refFWHM,GAP)
...
gaussLBMM     = 0.04 * dataToFit.txfrq*1e-6; % Common Gaussian dampening [Hz]
...
x0          = [ph0; ph1; gaussLB; gaussLBMM; lorentzLB; freqShift; ampl; lineShape];
...
lb = [lb_ph0; lb_ph1; lb_gaussLB; lb_gaussLBMM; lb_lorentzLB_mets; lb_lorentzLB_MM; lb_freqShift_mets; lb_freqShift_MM; lb_ampl; lb_lineShape];
ub = [ub_ph0; ub_ph1; ub_gaussLB; ub_gaussLBMM; ub_lorentzLB_mets; ub_lorentzLB_MM; ub_freqShift_mets; ub_freqShift_MM; ub_ampl; ub_lineShape];
...
gaussLBMM     = x(4); % Gaussian dampening [Hz^2]
lorentzLB   = x(5:nBasisFcts+4); % Lorentzian dampening [Hz] for each basis function
freqShift   = x(nBasisFcts+5:2*nBasisFcts+4); % Frequency shift [Hz] for each basis function
ampl        = x(2*nBasisFcts+5:3*nBasisFcts+4+nSplines); % Amplitudes
lineShape   = x(3*nBasisFcts+5+nSplines:end); % Lineshape coefficients
...
for ii=resBasisSet.nMets+1:nBasisFcts
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) .* exp(-1i*freqShift(ii).*t)' .* exp(-lorentzLB(ii).*t)' .* exp(-gaussLBMM.*t.*t)';    
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) * exp(1i*ph0);
end
...
for ii=resBasisSet.nMets:nBasisFcts
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) .* exp(-1i*freqShift(ii).*t)' .* exp(-lorentzLB(ii).*t)' .* exp(-gaussLBMM.*t.*t)';    
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) * exp(1i*ph0);
end
resBasisSet.specs = fftshift(fft(resBasisSet.fids,[],1),1);
...
fitParamsFinal.gaussLBMM      = x(4);
fitParamsFinal.lorentzLB    = x(5:nBasisFcts+4);
fitParamsFinal.freqShift    = x(nBasisFcts+5:2*nBasisFcts+4);
fitParamsFinal.lineShape    = x(3*nBasisFcts+5+size(splineArray,2):end);
