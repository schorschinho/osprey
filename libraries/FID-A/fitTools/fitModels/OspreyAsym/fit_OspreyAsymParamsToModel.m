function [ModelOutput] = fit_OspreyAsymParamsToModel(inputData, inputSettings, fitParams)
...
t = basisSet.t;
for ii=1:nBasisFcts
    basisSet.fids(:,ii) = basisSet.fids(:,ii) .* exp(-1i*freqShift(ii).*t)' .* exp(-lorentzLB(ii).*t)'.* lineShape_function_TD;     
