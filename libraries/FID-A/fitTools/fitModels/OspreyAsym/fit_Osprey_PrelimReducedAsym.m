function [fitParamsStep1] = fit_Osprey_PrelimReducedAsym(dataToFit, basisSet, fitRangePPM)
...
metabList = fit_createMetabList(basisSet.name);
fitMM = 0;
...
[splineArray, ~]    = fit_makeSplineBasis(dataToFit, fitRangePPM, 0.067);
...
% gaussLB = refFWHM * dataToFit.txfrq*1e-6;
...
% Set up lineshape model
asym_factor = -((pi*gaussLB/2).^2) ./ (log(0.5));

t = resBasisSet.t;
for ii=1:nMets
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) .* exp(-1i*freqShift.*t)' .* exp(-asym_factor.*t.*t)';    
