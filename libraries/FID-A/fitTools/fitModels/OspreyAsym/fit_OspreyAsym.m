...
function [fitParams, resBasisSet] = fit_OspreyAsym(dataToFit, basisSet, fitOpts)
...
if ~isfield(fitOpts, 'MRSIpriors')
    ...
    [fitParamsStep1] = fit_Osprey_PrelimReducedAsym(dataToFitRef, resBasisSet, [1.8, 4]);
else
...
if isfield(fitOpts,'coMM3') && isfield(dataToFit,'refShift') && ~isfield(fitOpts,'Diff2')
    switch fitOpts.coMM3        
        ...
        otherwise
            [fitParamsStep2] = fit_OspreyAsymPrelimStep2(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1);
    end             
else
    [fitParamsStep2] = fit_OspreyAsymPrelimStep2(dataToFitRef, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1);    
end
