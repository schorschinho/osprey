function [fitParamsStep2] = fit_OspreyAsymPrelimStep2(dataToFit, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1)
%% [fitParamsStep2] = fit_OspreyAsymPrelimStep2(dataToFit, resBasisSet, minKnotSpacingPPM, fitRangePPM, fitParamsStep1)
...
lorentzLB   = fitParamsStep1.freqShift * zeros(nBasisFcts,1); % Frequency shift [Hz] for each basis function
...
asym        = 0; % Asymmetry factor for the lineshape model 


% Concatenate all initial guesses together into one large x0 vector.
x0          = [ph0; ph1; gaussLB; lorentzLB; freqShift; ampl; asym];
...
lb_gaussLB          = 0; 
ub_gaussLB          = 15; % Gaussian dampening [Hz]
...
lb_asym        = -0.25;
ub_asym        = +0.25; % Lineshape asymmetry (see Wilson 2020 ABFit paper

% Concatenate together into LB/UB vectors
lb = [lb_ph0; lb_ph1; lb_gaussLB; lb_lorentzLB_mets; lb_lorentzLB_MM; lb_freqShift_mets; lb_freqShift_MM; lb_ampl; lb_asym];
ub = [ub_ph0; ub_ph1; ub_gaussLB; ub_lorentzLB_mets; ub_lorentzLB_MM; ub_freqShift_mets; ub_freqShift_MM; ub_ampl; ub_asym];
...
asym   = x(3*nBasisFcts+4+nSplines:end); % Lineshape coefficients
...
% (frequency shift, Lorentzian dampening, zero phase shift)
t = resBasisSet.t;

% Set up lineshape function as parameter of frequency
% Create a ppm vector around a pivot point (water)
ppm_ax = resBasisSet.ppm;
pivotPoint = 4.68;
multiplier = ppm_ax - pivotPoint;

asym_factor = 2*gaussLB ./ (1 + exp(asym .* multiplier));
lineShape_function = sqrt(4*(log(2)/pi)) .* exp(- (4 * log(2) .* (multiplier ./ asym_factor)));
lineShape_function_TD = ifft(fftshift(lineShape_function,1),[],1);

for ii=1:nBasisFcts
    resBasisSet.fids(:,ii) = resBasisSet.fids(:,ii) .* exp(-1i*freqShift(ii).*t)' .* exp(-lorentzLB(ii).*t)' .* lineShape_function_TD;    
...
fitParamsFinal.asym         = x(3*nBasisFcts+4+size(splineArray,2):end);
