...
for rr = 1 : Voxels
    for ss = 1 : NoSubSpec %loop over subspectra
        ...
        for g = 1 : length(names) % loop over groups
            tempSubSpec = zeros(length(MRSCont.overview.Osprey.(['sort_data_voxel_' num2str(rr)]).(names{g}).(SubSpecNames{ss})),MRSCont.overview.Osprey.(['all_data_voxel_' num2str(rr)]).(SubSpecNames{1}){1,1}.sz(1));
        ...
        %Store ppm
        MRSCont.overview.Osprey.(['ppm_data_' SubSpecNames{ss}]) = MRSCont.overview.Osprey.(['all_data_voxel_' num2str(rr)]).(SubSpecNames{ss}){1,1}.ppm;
    end
end
