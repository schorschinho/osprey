...
for sf = 1 : NoFit
    if ~strcmp([FitNames{sf} '_' dataPlotNames{sf}], 'ref_ref') || strcmp([FitNames{sf} '_' dataPlotNames{sf}], 'w_w')
        for kk = 1 : MRSCont.nDatasets
            ...
            ppmmax=ppmrange(ppmindex);
            MRSCont.overview.Osprey.refShift(kk)=(ppmmax-2.013);
    ...
    if isfield(MRSCont, 'quantify')
        ...
        if MRSCont.flags.isMEGA
                if isfield(MRSCont.overview.Osprey.all_models, 'conc_diff1')
                    ...
                    MRSCont.overview.Osprey.all_data.B{1,kk}.specs= MRSCont.overview.Osprey.all_data.B{1,kk}.specs/(MRSCont.fit.results.diff1.fitParams{1,kk}.ampl(idx_NAA)+ MRSCont.fit.results.diff1.fitParams{1,kk}.ampl(idx_Cr))/MRSCont.fit.scale{kk};
...
for ss = 1 : NoSubSpec
    ...
    for g = 1 : length(names)
        ...
        for kk = 1 : length(MRSCont.overview.Osprey.sort_data.(names{g}).(SubSpecNames{ss}))
...
for sf = 1 : NoFit
    ...
    for g = 1 : length(names)
            ...
            for kk = 1 : length(MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]))
              tempSubSpec(kk,:) = MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]){1,kk}.fit;
              tempSubRes(kk,:) = MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]){1,kk}.res;
              tempSubdata(kk,:) = MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]){1,kk}.data;
              if ~(strcmp([FitNames{sf} '_' dataPlotNames{sf}], 'ref_ref') || strcmp([FitNames{sf} '_' dataPlotNames{sf}], 'w_w'))
                tempSubBaseline(kk,:) = MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]){1,kk}.baseline;
                fits = fields(MRSCont.overview.Osprey.sort_fit.(names{g}).([FitNames{sf} '_' dataPlotNames{sf}]){1,kk});
                 for f = 6 : length(fits)
