...
data_folder = fileparts(which(fullfile('exampledata','rda','jobRDA.m'))); 
...
for kk = 1:length(subs)
    ...
    for ll = 1:length(sess)
        ...
        % (MANDATORY)
        dir_metabolite    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_press' filesep '*.rda']);
        ...
        % default.
        dir_ref    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_press-ref' filesep '*.rda']);
...
% Specify metabolite data
% (MANDATORY)
% files(counter)      = {'/Volumes/MyProject/data/sub-01/mrs/PRESS_act.rda',...
%                        '/Volumes/MyProject/data/sub-02/mrs/PRESS_act.rda'};

% Specify water reference data for eddy-current correction (same sequence as metabolite data!)
% (OPTIONAL)
% Leave empty for GE P-files (.7) - these include water reference data by
% default.
% files_ref(counter)      = {'/Volumes/MyProject/data/sub-01/mrs/PRESS_ref.rda',...
%                            '/Volumes/MyProject/data/sub-02/mrs/PRESS_ref.rda'};
