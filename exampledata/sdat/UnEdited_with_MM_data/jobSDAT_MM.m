...
clear files files_ref files_w files_nii files_mm files_mm_ref
...
data_folder = fileparts(which(fullfile('exampledata','sdat','UnEdited_with_MM_data','jobSDAT_MM.m')));
...
for kk = 1:length(subs)
    ...
    for ll = 1:length(sess)
        ...
        % default.
        dir_ref    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_press_ref' filesep '*.SDAT']);
        files_ref(counter)  = {[dir_ref(end).folder filesep dir_ref(end).name]};

        % Specify metabolite-nulled data to estimate macromolecules
        % (OPTIONAL)
        dir_mm    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_press-inverted' filesep '*.SDAT']);
        files_mm(counter)  = {[dir_mm(end).folder filesep dir_mm(end).name]};

        % Specify water reference for eddy-current correction for metabolite-nulled data to estimate macromolecules
        % (OPTIONAL)
        % Leave empty for GE P-files (.7) - these include water reference data by
        % default.
        dir_mm_ref    = dir([sess(ll).folder filesep sess(ll).name filesep 'mrs' filesep subs(kk).name '_' sess(ll).name '_press-inverted_ref' filesep '*.SDAT']);
        files_mm_ref(counter)  = {[dir_mm_ref(end).folder filesep dir_mm_ref(end).name]};
       ...
       % Specify T1-weighted structural imaging data
        ...
        files_nii(counter)  = {[sess(ll).folder filesep sess(ll).name filesep 'anat' filesep subs(kk).name '_' sess(ll).name '_T1w_scanner-space_FSL-skull-stripped.nii.gz']};
