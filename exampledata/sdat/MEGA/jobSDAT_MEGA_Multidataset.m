...
for kk = 1:length(subs)
    ...
    for ll = 1:length(sess)
        ...
        % (MANDATORY)
        % This is a multiple scans per subject example where we duplicate
        % the scan entries from the example data. These can then be
        % averaged using the osp_AverageAllDatasetsAlongExtra function
        ...
        files(counter)      = {{[dir_metabolite(end).folder filesep dir_metabolite(end).name], [dir_metabolite(end).folder filesep dir_metabolite(end).name]}};
        ...
        files_ref(counter)  = {{[dir_ref(end).folder filesep dir_ref(end).name], [dir_ref(end).folder filesep dir_ref(end).name]}};
...
% Specify metabolite data
% (MANDATORY)
% files                 = {{'/Volumes/MyProject/data/sub-01/mrs/MEGAPRESS_act.SDAT',...
%                        '/Volumes/MyProject/data/sub-02/mrs/MEGAPRESS_act.SDAT'},...
%                        {'/Volumes/MyProject/data/sub-01/mrs/MEGAPRESS_act.SDAT',...
%                        '/Volumes/MyProject/data/sub-02/mrs/MEGAPRESS_act.SDAT'}};
...
% Leave empty for GE P-files (.7) - these include water reference data by
% default.
% files_ref      = {{'/Volumes/MyProject/data/sub-01/mrs/MEGAPRESS_ref.SDAT',...
%                        '/Volumes/MyProject/data/sub-02/mrs/MEGAPRESS_ref.SDAT'},...
%                        {'/Volumes/MyProject/data/sub-01/mrs/MEGAPRESS_ref.SDAT',...
%                        '/Volumes/MyProject/data/sub-02/mrs/MEGAPRESS_ref.SDAT'}};
...
%                        {'/Volumes/MyProject/data/sub-01/mrs/PRESS_ref.SDAT',...
%                        '/Volumes/MyProject/data/sub-02/mrs/PRESS_ref.SDAT'}};
...
% (MANDATORY)
outputFolder = fullfile(data_folder, 'derivatives_Multidatasets');
