...
opts.fit.ModelProcedure.metab   = {which(fullfile('Osprey_gLCM','fitClass','model-procedures','defaults','3Step_Spline_invivo_HERMES-sum_Reg_Optim_Full_soft_constraint.json')),...                                 
                                   which(fullfile('Osprey_gLCM','fitClass','model-procedures','defaults','3Step_Spline_invivo_HERMES-diff1_Reg_Optim_Full_soft_constraint.json')),...
                                   which(fullfile('Osprey_gLCM','fitClass','model-procedures','defaults','3Step_Spline_invivo_HERMES-diff2_Reg_Optim_Full_soft_constraint.json'))};
...
% opts.fit.basisSetFile = 'osprey/fit/basis/3T/siemens/hermes/gabagsh/basis_siemens_hermes.mat';
...
data_folder = fullfile(oneDriveFolder, 'OspreyTestData', 'siemens', 'hermes');

% Specify metabolite data
% (MANDATORY)
files    = {fullfile(data_folder, 'Herc_02_HERMES.dat')};

% Specify water reference data for eddy-current correction (same sequence as metabolite data!)
% (OPTIONAL)
% Leave empty for GE P-files (.7) - these include water reference data by
% default.
files_ref    = {fullfile(data_folder, 'Herc_02_HERMES_water.dat')};
