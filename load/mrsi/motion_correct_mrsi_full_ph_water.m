function [k_sort_on, k_sort_on2, k_sort_off, k_sort_off2,....
    ...
    off2_replace_track, zero_replace_track, k_space_locs, corr_options] = motion_correct_mrsi_full_ph_water(k_sort_on, k_sort_on2, k_sort_off, k_sort_off2,....
                                                                                                on_spec_k_1, on_spec_k_2, off_spec_k_1, off_spec_k_2,...
...
on1_replace_track = zeros(size(k_sort_on2,1),size(k_sort_on2,2));
off1_replace_track = zeros(size(k_sort_on2,1),size(k_sort_on2,2));
on2_replace_track = zeros(size(k_sort_on2,1),size(k_sort_on2,2));
off2_replace_track = zeros(size(k_sort_on2,1),size(k_sort_on2,2));

zero_replace_track = zeros(size(k_sort_on2,1),size(k_sort_on2,2));

k_space_locs = zeros(size(k_sort_on2,1),size(k_sort_on2,2));
...
for this_kx = 1:size(on_spec_k_1,1)
    for this_ky = 1:size(on_spec_k_1,2)
        ...
        % ranges.
        this_on1_wat = real(squeeze(on_spec_k_1( this_kx, this_ky, met_start_wat:met_end_wat)));
        this_off1_wat = real(squeeze(off_spec_k_1( this_kx, this_ky, met_start_wat:met_end_wat)));
        this_on2_wat = real(squeeze(on_spec_k_2( this_kx, this_ky, met_start_wat:met_end_wat)));
        this_off2_wat = real(squeeze(off_spec_k_2( this_kx, this_ky, met_start_wat:met_end_wat)));

        this_on1_wat_im = squeeze(on_spec_k_1( this_kx, this_ky, met_start_wat:met_end_wat));
        this_off1_wat_im = squeeze(off_spec_k_1( this_kx, this_ky, met_start_wat:met_end_wat));
        this_on2_wat_im = squeeze(on_spec_k_2( this_kx, this_ky, met_start_wat:met_end_wat));
        this_off2_wat_im = squeeze(off_spec_k_2( this_kx, this_ky, met_start_wat:met_end_wat));
        ...
        if all(this_on1_wat)% && all(sum([this_kx this_ky] ~= [round(kx_tot/2) round(ky_tot/2)]))
            ...
            all_p = [mean([p_wat(1)]), mean([p_wat(2)]),...
                        mean([p_wat(3)]), mean([p_wat(4)]),... % p_crcho(4)
                        mean([p_wat(5)]), mean([p_wat(6)])];

            all_p_thresh = [all_p_thresh_wat(1),...% | all_p_thresh_crcho(1),...
                            all_p_thresh_wat(2:5),...% | all_p_thresh_crcho(2:5),...
                            all_p_thresh_wat(6)];% | all_p_thresh_crcho(6)];% | all_p_thresh_lip;

            % Last resort, all of them don't match, unless one pair is > 0.6          
            all_p_thresh_wat_zero = p_wat < last_resort_thresh;


            all_p_thresh_zero = [all_p_thresh_wat_zero(1),...% | all_p_thresh_crcho_zero(1),...
                            all_p_thresh_wat_zero(2:5),...% | all_p_thresh_crcho_zero(2:5),...
                            all_p_thresh_wat_zero(6)];% | all_p_thresh_crcho_zero(6)];% | all_p_thresh_lip;
            ...
            all_spec = [squeeze(on_spec_k_1( this_kx, this_ky, :)), squeeze(on_spec_k_2( this_kx, this_ky, :)),...
                        squeeze(off_spec_k_1( this_kx, this_ky, :)), squeeze(off_spec_k_2( this_kx, this_ky, :))];
            ...
            if all(all_p_thresh_zero) % All sub-acquisitions are different from each other
                ...
                zero_replace_track(this_kx,this_ky) = 1;
            else
                if all(all_p_thresh == [1 0 0 0 0 0]) % [1] Phase or replace ON1 OR ON2
                    ...
                    if keep == 1 %Phase ON2 or replace ON2
                        [corr_ph, ph_corr_coeff] = ph_k_space([0 1 0 0], all_spec_wat, all_spec_wat, all_spec);
                        
                        if ph_corr_coeff > ph_thresh
                            k_ph_corr_on2(this_kx, this_ky) = corr_ph;
                        else
                            k_sort_on2(this_kx,this_ky,:,:) = k_sort_on(this_kx,this_ky,:,:);
                            on_spec_k_2(this_kx,this_ky,:) = on_spec_k_1(this_kx,this_ky,:,:);
                            on2_replace_track(this_kx,this_ky) = 1;
                        end
                    elseif keep == 2 %Phase ON1 or replace ON1
                        [corr_ph, ph_corr_coeff] = ph_k_space([1 0 0 0], all_spec_wat, all_spec_wat, all_spec);
                        
                        if ph_corr_coeff > ph_thresh
                            k_ph_corr_on1(this_kx, this_ky) = corr_ph;
                        else
                            k_sort_on(this_kx,this_ky,:,:) = k_sort_on2(this_kx,this_ky,:,:);
                            on_spec_k_1(this_kx,this_ky,:) = on_spec_k_2(this_kx,this_ky,:,:);
                            on1_replace_track(this_kx,this_ky) = 1;
                        end
                    end
                ...
                elseif all(all_p_thresh == [1 1 1 1 0 0]) % [4] Replace ON1 w/ON2, and replace OFF1 with OFF2
                    [corr_ph, ph_corr_coeff] = ph_k_space([1 0 1 0], all_spec_wat, all_spec_wat, all_spec);
                    corr_options = [corr_options, 4];
                    
                    if ph_corr_coeff(1) > ph_thresh
                        k_ph_corr_on1(this_kx, this_ky) = corr_ph(1);
                    else
                        k_sort_on(this_kx,this_ky,:,:) = k_sort_on2(this_kx,this_ky,:,:);
                        on_spec_k_1(this_kx,this_ky,:) = on_spec_k_2(this_kx,this_ky,:,:);
                        on1_replace_track(this_kx,this_ky) = 1;
                ...
                elseif all(all_p_thresh == [1 1 1 1 1 1]) % [6] Pick the best ON/OFF pair
                    ...
                    if keep == 1 %Replace ON2, OFF2
                        [corr_ph, ph_corr_coeff] = ph_k_space([0 1 0 1], all_spec_wat, all_spec_wat, all_spec);
                        if ph_corr_coeff(1) > ph_thresh
                            k_ph_corr_on2(this_kx, this_ky) = corr_ph(1);
                        else
                            k_sort_on2(this_kx,this_ky,:,:) = k_sort_on(this_kx,this_ky,:,:);
                            on_spec_k_2(this_kx,this_ky,:) = on_spec_k_1(this_kx,this_ky,:,:);
                            on2_replace_track(this_kx,this_ky) = 1;
                        ...
                        if ph_corr_coeff(2) > ph_thresh
                            k_ph_corr_off2(this_kx, this_ky) = corr_ph(2);
                        else
                    ...
                    elseif keep == 2 %Replace ON2, OFF1
                        [corr_ph, ph_corr_coeff] = ph_k_space([0 1 1 0], all_spec_wat, all_spec_wat, all_spec);
                        if ph_corr_coeff(1) > ph_thresh
                            k_ph_corr_on2(this_kx, this_ky) = corr_ph(1);
                        else
                            k_sort_on2(this_kx,this_ky,:,:) = k_sort_on(this_kx,this_ky,:,:);
                    ...
                    elseif keep == 3 %Replace ON1, OFF2
                        [corr_ph, ph_corr_coeff] = ph_k_space([1 0 0 1], all_spec_wat, all_spec_wat, all_spec);
                        if ph_corr_coeff(1) > ph_thresh
                            k_ph_corr_on1(this_kx, this_ky) = corr_ph(1);
                        else
                            k_sort_on(this_kx,this_ky,:,:) = k_sort_on2(this_kx,this_ky,:,:);
                ...
                elseif all(all_p_thresh == [0 1 0 0 0 0]) % [7]. Replace ON1 or OFF1
                    ...
                    if keep == 1 %Replace OFF1
                        [corr_ph, ph_corr_coeff] = ph_k_space([0 0 1 0], all_spec_wat, all_spec_wat, all_spec);
                        if ph_corr_coeff > ph_thresh
                            k_ph_corr_off1(this_kx, this_ky) = corr_ph;
                        else
                            k_sort_off(this_kx,this_ky,:,:) = k_sort_off2(this_kx,this_ky,:,:);
                            off_spec_k_1(this_kx,this_ky,:) = off_spec_k_2(this_kx,this_ky,:,:); 
                            off1_replace_track(this_kx,this_ky) = 1;
                ...
                elseif all(all_p_thresh == [0 1 1 1 1 0]) % [10] Pick the best ON/OFF pair
                    ...
                    if keep == 1 %Replace ON2, OFF2
                        ...
                        if ph_corr_coeff(2) > ph_thresh
                            ...
                            k_sort_off2(this_kx,this_ky,:,:) = k_sort_off(this_kx,this_ky,:,:);
                            off_spec_k_2(this_kx,this_ky,:) = off_spec_k_1(this_kx,this_ky,:,:); 
                            off2_replace_track(this_kx,this_ky) = 1;
                        end
                ...
                elseif all(all_p_thresh == [0 0 1 0 1 0]) % [20] Replace OFF2
                    [corr_ph, ph_corr_coeff] = ph_k_space([0 0 0 1], all_spec_wat, all_spec_wat, all_spec);
                    corr_options = [corr_options, 20];
                    
                    if ph_corr_coeff > ph_thresh
                        k_ph_corr_off2(this_kx, this_ky) = corr_ph;
                    else
                        k_sort_off2(this_kx,this_ky,:,:) = k_sort_off(this_kx,this_ky,:,:);
                        off_spec_k_2(this_kx,this_ky,:) = off_spec_k_1(this_kx,this_ky,:,:); 
                        off2_replace_track(this_kx,this_ky) = 1;
                ...
                elseif all(all_p_thresh == [1 1 1 0 1 1]) % [62] Replace ON1, OFF2
                        ...
                        if ph_corr_coeff(2) > ph_thresh
                            ...
                            k_sort_off(this_kx,this_ky,:,:) = k_sort_off2(this_kx,this_ky,:,:);
                            off_spec_k_2(this_kx,this_ky,:) = off_spec_k_2(this_kx,this_ky,:,:); 
                            on2_replace_track(this_kx,this_ky) = 1;
                        end
