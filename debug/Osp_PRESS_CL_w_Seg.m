...
function tests = Osp_PRESS_CL_w_Seg
...
function testOspreyJob(~)
    ...
    dir = strrep(which(['debug' filesep 'jobPRESS.m']),'jobPRESS.m','derivatives');
    if ~isempty(dir)
        delete(fullfile(dir,'jobPRESS.mat'));
        delete(fullfile(dir,'LogFile.txt'));
    end
    MRSCont = OspreyJob(which(['debug' filesep 'jobPRESS.m']),0,'11');
    addpath(dir);
end

%Test OspreyLoad
function testOspreyLoad(~)
    load(which(['debug' filesep 'derivatives' filesep 'jobPRESS.mat']));
